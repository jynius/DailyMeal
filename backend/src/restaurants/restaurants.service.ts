...
export class RestaurantsService {
  ...
  async getRestaurantsFromMeals(userId: string): Promise<RestaurantSummary[]> {
    ...
    restaurantMap.forEach((data, location) => {
      ...
      const representativeMeal = data.meals.find(m => m.photos && m.photos.length > 0);
      
      // 가격대 계산 (평균 가격 기준)
      const avgPrice = data.meals
        .filter(m => m.price)
      ...
      restaurants.push({
        ...
        representativePhoto: representativeMeal?.photos?.[0] ? `/uploads/${representativeMeal.photos[0]}` : undefined,
        priceRange
      });
  ...
  private inferCategory(location: string, meals: MealRecord[]): string {
    ...
    if (locationLower.includes('이탈리안') || mealNames.includes('파스타') || mealNames.includes('피자')) return '양식';
    if (locationLower.includes('일식') || mealNames.includes('스시') || mealNames.includes('라멘')) return '일식';
    if (locationLower.includes('중식') || mealNames.includes('짜장') || mealNames.includes('짬뽕')) return '중식';
    if (mealNames.includes('김치') || mealNames.includes('비빔밥') || mealNames.includes('된장')) return '한식';
  ...
  async createRestaurantMap(
    ...
    const averageRating = selectedRestaurants.reduce((sum, r) => sum + r.averageRating, 0) / selectedRestaurants.length;
    const categories = Array.from(new Set(selectedRestaurants.map(r => r.category).filter((c): c is string => Boolean(c))));
