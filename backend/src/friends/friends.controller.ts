...
export class FriendsController {
  ...
  // 친구 요청 보내기
  @Post('request')
  async sendFriendRequest(@Request() req: AuthenticatedRequest, @Body('email') email: string) {
    return this.friendsService.sendFriendRequest(req.user.id, email)
  }

  // 친구 요청 수락
  @Post(':id/accept')
  async acceptFriendRequest(@Request() req: AuthenticatedRequest, @Param('id') id: string) {
    return this.friendsService.acceptFriendRequest(id, req.user.id)
  }

  // 친구 요청 거절
  @Post(':id/reject')
  async rejectFriendRequest(@Request() req: AuthenticatedRequest, @Param('id') id: string) {
  ...
  // 친구 요청 취소 (보낸 요청)
  @Delete('requests/:id')
  async cancelFriendRequest(@Request() req: AuthenticatedRequest, @Param('id') id: string) {
  ...
  // 친구 삭제
  @Delete(':friendId')
  async removeFriend(@Request() req: AuthenticatedRequest, @Param('friendId') friendId: string) {
  ...
  // 친구 검색 (이메일 또는 이름)
  @Get('search')
  async searchUsers(@Request() req: AuthenticatedRequest, @Query('query') query: string) {
  ...
  // 친구 검색 (이메일) - 구버전 호환
  @Get('search/:email')
  async searchUserByEmail(@Request() req: AuthenticatedRequest, @Param('email') email: string) {
