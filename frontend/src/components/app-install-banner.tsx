...
export function AppInstallBanner() {
  ...
  useEffect(() => {
    ...
    const isPWA =
      ...
      (globalThis.window.navigator as any).standalone === true // iOS Safari 지원
    ...
    const shouldShowAgain = dismissedTime
      ? Date.now() - Number.parseInt(dismissedTime) > 7 * 24 * 60 * 60 * 1000
      : true
      ...
      <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs opacity-90 truncate">더 빠르고 편리한 식사 기록</p>
          </div>
        </div>
